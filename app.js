...
// ✅ AGENDADO? = S: não mostra horários (apenas badge + contatos/endereço/obs)
// ✅ Telefone (tel:) e e-mail (mailto:) clicáveis
...
// Extrai todos HH:mm presentes no texto
...
// tel: (assumindo BR quando não vier com DDI)
function phoneToTelHref(raw) {
  const digits = sanitizePhone(raw);
  if (!digits) return null;

  // Se já veio com 55 (DDI), usa como está
  if (digits.startsWith("55")) return `tel:+${digits}`;

  // Se veio com 10/11 dígitos (DDD + número), adiciona +55
...
// período por horários reais (corte 13:00)
function rowHasMorning(row) {
  const manha = getField(row, ["manha", "MANHA", "MANHÃ", "Manhã", "Manha"]);
  const tarde = getField(row, ["tarde", "TARDE", "Tarde"]);
  const all = [
    ...splitIntoRangesWithStart(manha),
    ...splitIntoRangesWithStart(tarde),
  ];
...
// card com manhã e tarde no mesmo dia
function makeTimeBlockSmart(row) {
  ...
  const morningList = all
    .filter((r) => r.startMins < MIN_TARDE)
    .map((r) => r.label);
  const afternoonList = all
    .filter((r) => r.startMins >= MIN_TARDE)
    .map((r) => r.label);
  ...
  if (morningList.length) {
    ...
    rowEl.innerHTML = `<span class="time-label">Manhã</span><span class="time-value">${morningList.join(
      " • "
    )}</span>`;
  ...
  if (afternoonList.length) {
    const rowEl = document.createElement("div");
    rowEl.className = "time-row";
    rowEl.innerHTML = `<span class="time-label">Tarde</span><span class="time-value">${afternoonList.join(
      " • "
    )}</span>`;
    wrap.appendChild(rowEl);
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const periodButtons = Array.from(document.querySelectorAll("[data-p]")).filter(
    (b) => ["AMBOS", "MANHA", "TARDE"].includes((b.dataset.p || "").toUpperCase())
  );

  function setPeriodoActiveUI(key) {
    periodButtons.forEach((b) =>
      b.classList.toggle("active", (b.dataset.p || "").toUpperCase() === key)
    );
  }
  ...
  function setUpdatedAtText(payload) {
    ...
    const total =
      typeof meta.total_registros === "number" ? meta.total_registros : data.length;
  ...
  function render() {
    ...
    const filtered = data
      ...
      // período só faz sentido para NÃO agendados (agendados já foram filtrados)
      .filter((r) => matchPeriodo(r, periodo))
      .filter((r) => matchesQuery(r, q))
      .sort((a, b) =>
        (a.medico_nome || "").localeCompare(b.medico_nome || "", "pt-BR", {
          sensitivity: "base",
        })
      );
    ...
    filtered.forEach((r) => {
      ...
      // Badge só no filtro S (porque só aparece lá)
      if (agFilter === "S") {
      ...
      // ✅ se for agendado: NÃO mostra horários
      let timeBlock = document.createElement("div");
      if (!isAgendado(r)) {
        timeBlock = makeTimeBlockSmart(r);
      }

      // Pills com links (só se preenchido)
      ...
      if (r.bairro) {
        ...
        p.className = "pill";
        p.textContent = `Bairro: ${r.bairro}`;
        pills.appendChild(p);
      }

      const telHref = phoneToTelHref(r.telefone);
      if (r.telefone && telHref) {
        const a = document.createElement("a");
        a.className = "pill pill-link";
        a.href = telHref;
        a.textContent = `Tel: ${r.telefone}`;
        pills.appendChild(a);
      } else if (r.telefone) {
        const p = document.createElement("span");
        p.className = "pill";
        p.textContent = `Tel: ${r.telefone}`;
        pills.appendChild(p);
      }

      const celHref = phoneToTelHref(r.celular);
      if (r.celular && celHref) {
        const a = document.createElement("a");
        a.className = "pill pill-link";
        a.href = celHref;
        a.textContent = `Cel: ${r.celular}`;
        pills.appendChild(a);
      } else if (r.celular) {
        const p = document.create bytes
        p.className = "pill";
        p.textContent = `Cel: ${r.celular}`;
        pills.appendChild(p);
      }

      const mailHref = emailToHref(r.email);
      if (r.email && mailHref) {
        const a = document.createElement("a");
        a.className = "pill pill-link";
        a.href = mailHref;
        a.textContent = `E-mail: ${r.email}`;
        pills.appendChild(a);
      } else if (r.email) {
        const p = document.createElement("span");
        p.className = "pill";
        p.textContent = `E-mail: ${r.email}`;
        pills.appendChild(p);
      }
  ...
  async function load() {
    try {
      ...
      if ($list)
        $list.innerHTML = `<div class="empty">Erro: ${String(e.message || e)}<br><br>
